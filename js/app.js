(() => {
    ...
    function parallax() {
        const elements = document.querySelectorAll("[data-paralax]");
        const scrollPosition = window.pageYOffset;
        elements.forEach((element => {
            const paralaxMultiply = parseInt(element.getAttribute("data-paralax")) / 100 || .5;
            element.style.transform = `translateY(${scrollPosition * paralaxMultiply}px)`;
        }));
    }
    const bufuerPopup = document.querySelector(".bufer");
    ...
    function windowLoad() {
        ...
        let sections = document.querySelectorAll(".about__numbers");
        if (sections.length) sections.forEach((section => {
            observer.observe(section);
        }));
        setTimeout((() => {
            bufuerPopup.classList.add("_loaded");
        }), 100);
    }
    const popularItems = document.querySelectorAll(".popular__spollers_item").length;
    const popularItemsImgs = document.querySelectorAll(".popular__spollers_body-img");
    const popularItemsButtons = document.querySelectorAll(".popular__spollers_body-button");
    for (let i = 0; i < popularItems; i++) {
        popularItemsButtons[i].addEventListener("mouseover", (function() {
            popularItemsImgs[i].style.transform = "scale(1.05)";
        }));
        popularItemsButtons[i].addEventListener("mouseout", (function() {
    ...
    document.querySelector("[data-bufer]").addEventListener("click", (function() {
        ...
        clipboard.on("success", (function(e) {
            bufuerPopup.classList.add("_active");
            setTimeout((function() {
                bufuerPopup.classList.remove("_active");
            }), 3e3);
            e.clearSelection();
        }));
